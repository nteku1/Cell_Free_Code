...
for iiii = 1:6000
    ...
    for n = 1:nbrOfSetups
       ...
       options = optimoptions(@fmincon,'MaxFunEvals',4500,'StepTolerance',1e-10,'TolCon',1e-10); 
      ...
      %referencing external cost function fun to calculate latency, default
       alphasss = fmincon(@(alphasss)fun_ALT_L2(alphasss,per_user_interference,all_states,L),alphasss_0,A,b,Aeq,beq,lb,ub,[],options);
    ...
    %iiii
      ...
      total_interference = sum( sum(total_transmission));
      per_user_interference = total_interference - total_transmission;
      outputs(iiii/K) =  fun_ALT_L2(alphasss,per_user_interference,all_states,L);
